...
//
// Postgres port note: every exported function here is now async (the
// node-postgres driver, unlike better-sqlite3, is inherently async — there
// is no `.all()`/`.run()`/`.get()`). Callers (the /api handlers) must
// `await` these.
...
/**
 * Full-text search over `search_blob` (see buildSearchBlob() in
 * json-columns.ts) using Postgres's `to_tsvector`/`plainto_tsquery`, backed
 * by a GIN expression index (migrations/0000). `plainto_tsquery` already
 * degrades gracefully for punctuation-only/empty input (matches nothing
 * rather than erroring), so — unlike the old SQLite FTS5 MATCH syntax —
 * there is no need for a separate sanitize-or-fallback path.
 */
async function searchMatchingIds(query: string): Promise<Set<number>> {
  const rows = await db.execute<{ id: number }>(
    sql`SELECT id FROM opportunities WHERE to_tsvector('english', search_blob) @@ plainto_tsquery('english', ${query})`
  );
 ...
 * name/description/majors/details/tags. Exported so scrapers (e.g. vip.ts,
 * which upserts via raw db calls rather than these helpers) can keep
 * newly-scraped rows searchable too.
 */
export async function refreshSearchBlob(opportunityId: number): Promise<void> {
  ...
  if (rows.length === 0) return;
  const row = rows[0];
  const tagLabels = await tagLabelsForOpportunity(opportunityId);
  ...
  await db.update(opportunities).set({ searchBlob: blob }).where(eq(opportunities.id, opportunityId));
}
...
export async function updateOpportunity(
  ...
  await db.transaction(async (tx) => {
    ...
    if (fields.tagSlugs !== undefined) {
      ...
      const ids =
        fields.tagSlugs.length > 0
          ? (await tx.select({ id: tags.id }).from(tags).where(inArray(tags.slug, fields.tagSlugs))).map(
              (r) => r.id
            )
          : [];
      for (const tagId of ids) {
        await tx.insert(opportunityTags).values({ opportunityId: id, tagId });
      }
    }
  });
